...
- Teste a conexão manualmente no phpMyAdmin

### 2. Erro: "Failed host lookup"

**Causa**: App não consegue acessar o backend

**Solução**:
- Verifique se o backend está rodando (`npm start`)
- Confira a URL no código do Flutter
- Use IP correto para dispositivo físico
...
**Solução**:
- Execute novamente `dbCreation.sql`
- Execute `dbInserts.sql`
- Verifique se o banco foi selecionado: `USE backend-mobile-ii;`

### 5. App mostra tela branca
...
**Causa**: URLs inválidas ou conexão com internet

**Solução**:
- Use URLs HTTPS válidas
- Teste as URLs no navegador
- Exemplo de URL funcional: `https://i.pravatar.cc/150?img=1`
...
3. ✅ App Flutter rodando
4. 📹 Grave o vídeo de apresentação (máx. 3 min)
5. 📝 Preencha o README com nomes dos integrantes
6. 🚀 Faça o commit no GitHub
7. 📤 Entregue no Moodle até 05/12
...
- Sempre inicie o backend ANTES do app Flutter
- Use `flutter doctor` para verificar problemas
- Teste primeiro no emulador, depois no dispositivo físico
- Salve seu trabalho frequentemente no Git
- Documente qualquer problema e solução encontrados
